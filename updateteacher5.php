...
if(isset($_POST['teacherid']) && isset($_POST['teacherage']) ){
...
if ( isset($teacherid) && isset($teacherage) ) {
$sql = "UPDATE teacher SET TEACHER_AGE='$teacherage' WHERE TEACHER_ID=$teacherid";
}
...
<!DOCTYPE html>
<head>
  <title>UPDATING TEACHER_AGE IN TEACHER TABLE</title>
</head>
<body>
  ...
  <h3 style="font-size:150%;font-family:algerian;"><i>UPDATING TEACHER_AGE IN TEACHER TABLE</i></h3><br></th></tr>
</table>
...
if (mysqli_query($conn, $sql)) {
  echo ('<h2 style="color: green;">'."TEACHER_AGE UPDATED AS $teacherage FOR TEACHER_ID $teacherid"."</h2>\n");
} else {
 ...
 ?>
  ...
  <tr><th>
   <form method="POST" style="background:lightyellow;" >
    <label for="teacherid">TEACHER_ID WHOSE AGE NEED TO BE CHANGED:</label><br><br>
    <input type="text" id="teacherid" name="teacherid"><br><br>
    <label for="teacherage">NEW TEACHER_AGE:</label><br><br>
