...
if(isset($_POST['parentid']) && isset($_POST['parentmobile']) ){
...
if ( isset($parentid) && isset($parentmobile) ) {
$sql = "UPDATE parent SET PARENT_MOBILE ='$parentmobile' WHERE PARENT_ID=$parentid";
}
...
<!DOCTYPE html>
<head>
  <title>UPDATING PARENT_MOBILE IN PARENT TABLE</title>
</head>
<body>
  ...
  <h3 style="font-size:150%;font-family:algerian;"><i>UPDATING PARENT_MOBILE IN PARENT TABLE</i></h3><br></th></tr>
</table>
...
if (mysqli_query($conn, $sql)) {
  echo ('<h2 style="color: green;">'."PARENT_MOBILE UPDATED AS $parentmobile FOR PARENT_ID $parentid"."</h2>\n");
} else {
 ...
 ?>
  ...
  <tr><th>
   <form method="POST" style="background:lightyellow;" >
    <label for="parentid">PARENT_ID WHOSE  MOBILE NUMBER TO BE CHANGED:</label><br><br>
    <input type="text" id="parentid" name="parentid"><br><br>
    <label for="parentmobile">NEW PARENT_MOBILE:</label><br><br>
