...
if(isset($_POST['parentid']) && isset($_POST['parentoccupation']) ){
...
if ( isset($parentid) && isset($parentoccupation) ) {
$sql = "UPDATE parent SET PARENT_OCCUPATION='$parentoccupation' WHERE PARENT_ID=$parentid";
}
...
<!DOCTYPE html>
<head>
  <title>UPDATING PARENT_OCCUPATION IN PARENT TABLE</title>
</head>
<body>
  ...
  <h3 style="font-size:150%;font-family:algerian;"><i>UPDATING PARENT_OCCUPATION IN PARENT TABLE</i></h3><br></th></tr>
</table>
...
if (mysqli_query($conn, $sql)) {
  echo ('<h2 style="color: green;">'."PARENT_OCCUPATION UPDATED AS $parentoccupation FOR PARENT_ID $parentid"."</h2>\n");
} else {
 ...
 ?>
  ...
  <tr><th>
   <form method="POST" style="background:lightyellow;" >
    <label for="parentid">PARENT_ID WHOSE OCCUPATION NEED TO BE CHANGED:</label><br><br>
    <input type="text" id="parentid" name="parentid"><br><br>
    <label for="parentoccupation">NEW PARENT_OCCUPATION:</label><br><br>
