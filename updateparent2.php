...
if(isset($_POST['parentid']) && isset($_POST['parentname']) ){
...
$sql = "UPDATE parent SET PARENT_NAME='$parentname' WHERE PARENT_ID=$parentid";
}
...
<!DOCTYPE html>
<head>
  <title>UPDATING PARENT_NAME IN PARENT TABLE</title>
</head>
<body>
  ...
  <h3 style="font-size:150%;font-family:algerian;"><i>UPDATING PARENT_NAME IN PARENT TABLE</i></h3><br></th></tr>
</table>
...
if (mysqli_query($conn, $sql)) {
  echo ('<h2 style="color: green;">'."PARENT_NAME UPDATED AS $parentname FOR PARENT_ID $parentid"."</h2>\n");
} else {
 ...
 ?>
  ...
  <tr><th>
   <form method="POST" style="background:lightyellow;" >
    <label for="parentid">PARENT_ID WHOSE NAME NEED TO BE CHANGED:</label><br><br>
    <input type="text" id="parentid" name="parentid"><br><br>
    <label for="parentname">NEW PARENT_NAME:</label><br><br>
