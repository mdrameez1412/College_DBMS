...
if(isset($_POST['parentid']) && isset($_POST['parentage']) ){
...
if ( isset($parentid) && isset($parentage) ) {
$sql = "UPDATE parent SET PARENT_AGE='$parentage' WHERE PARENT_ID=$parentid";
}
...
<!DOCTYPE html>
<head>
  <title>UPDATING PARENT_AGE IN PARENT TABLE</title>
</head>
<body>
  ...
  <h3 style="font-size:150%;font-family:algerian;"><i>UPDATING PARENT_AGE IN PARENT TABLE</i></h3><br></th></tr>
</table>
...
if (mysqli_query($conn, $sql)) {
  echo ('<h2 style="color: green;">'."PARENT_AGE UPDATED AS $parentage FOR PARENT_ID $parentid"."</h2>\n");
} else {
 ...
 ?>
  ...
  <tr><th>
   <form method="POST" style="background:lightyellow;" >
    <label for="parentid">PARENT_ID WHOSE AGE NEED TO BE CHANGED:</label><br><br>
    <input type="text" id="parentid" name="parentid"><br><br>
    <label for="parentage">NEW PARENT_AGE:</label><br><br>
